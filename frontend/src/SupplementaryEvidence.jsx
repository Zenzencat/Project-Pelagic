...
function SarObservation({ observation, label }) {
  return <figure style={{ margin: 0, minWidth: 0 }}>
    ...
    <img src={observation.overlay_preview} alt={`${label}, acquired ${observation.acquisition_start_utc}, VV SAR with segmentation overlay`}
...
export function ObservationEvidence({ evidence }) {
  ...
  return <details style={{ position: 'absolute', bottom: 16, left: 16, right: 16, zIndex: 1000,
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 260px), 1fr))', gap: 16 }}>
      <SarObservation observation={evidence.original} label="Original Sentinel-1" />
      {temporal?.status === 'available' && temporal.observations.map(observation =>
        <SarObservation key={observation.scene_id} observation={observation} label="Comparison Sentinel-1" />)}
      {optical?.status === 'available' && <figure style={{ margin: 0, minWidth: 0 }}>
        ...
        <img src={optical.observation.rgb_preview}
